...
class NsightProfiler:
    ...
    def register_layer_hooks(torch_model):
        ...
        target_keywords = ("self_attn", "block_sparse_moe")
...
if __name__ == "__main__":
    ...
    NsightProfiler.wrap_method(model, "generate", tag_name="generate")
    NsightProfiler.wrap_method(model, "mixtral_forward", tag_name="forward")
    
    if hasattr(model, "model") and isinstance(model.model, torch.nn.Module):
        # 2. 【新增】注册 Phase Tracker，用于区分 Prefill 和 Decode
        # 它会数 Forward 调用的次数：第0次=Prefill, >0=Decode
        print("Found internal PyTorch model, registering generation phase hooks...")
    ...
    try:
        model.generate(["Warmup"], output_token=1, input_token=1)
    except: pass
    print("Warmup Done.\n")
